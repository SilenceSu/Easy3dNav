...
import static com.github.silencesu.Easy3dNav.detour.Node.DT_NODE_CLOSED;
import static com.github.silencesu.Easy3dNav.detour.Node.DT_NODE_OPEN;
...
public class NavMeshQuery {
	...
	public FindRandomPointResult findRandomPoint(QueryFilter filter, FRand frand) {
		...
		float[] pt = DetourCommon.randomPointInConvexPoly(verts, poly.vertCount, areas, s, t);
	...
	public FindRandomPointResult findRandomPointAroundCircle(long startRef, float[] centerPos, float maxRadius,
		...
		while (!m_openList.isEmpty()) {
			...
			for (int i = bestPoly.firstLink; i != NavMesh.DT_NULL_LINK; i = bestTile.links.get(i).next) {
				...
				Tupple2<Float,Float> distseg = DetourCommon.distancePtSegSqr2D(centerPos, va, vb);
				...
				if (neighbourNode.flags == 0)
					neighbourNode.pos = DetourCommon.vLerp(va, vb, 0.5f);
		...
		float[] pt = DetourCommon.randomPointInConvexPoly(verts, randomPoly.vertCount, areas, s, t);
	...
	public ClosesPointOnPolyResult closestPointOnPoly(long ref, float[] pos) {
		...
		if (poly.getType() == Poly.DT_POLYTYPE_OFFMESH_CONNECTION) {
			...
			float u = d0 / (d0 + d1);
			float[] closest = DetourCommon.vLerp(tile.data.verts, v0, v1, u);
		...
		if (!DetourCommon.distancePtPolyEdgesSqr(pos, verts, nv, edged, edget)) {
			...
			int va = imin * 3;
			int vb = ((imin + 1) % nv) * 3;
			closest = DetourCommon.vLerp(verts, va, vb, edget[imin]);
			posOverPoly = false;
		} else {
			posOverPoly = true;
		}
		...
		if (tile.data.detailMeshes != null && tile.data.detailMeshes.length > ip) {
			...
			for (int j = 0; j < pd.triCount; ++j) {
				...
				Tupple2<Boolean, Float> heightResult = DetourCommon.closestHeightPointTriangle(closest, v[0], v[1], v[2]);
	...
	public float getPolyHeight(long ref, float[] pos) {
		...
		if (poly.getType() == Poly.DT_POLYTYPE_OFFMESH_CONNECTION) {
			...
			float d0 = DetourCommon.vDist2D(pos, v0);
			float d1 = DetourCommon.vDist2D(pos, v1);
			float u = d0 / (d0 + d1);
			...
			for (int j = 0; j < pd.triCount; ++j) {
				...
				Tupple2<Boolean, Float> heightResult = DetourCommon.closestHeightPointTriangle(pos, v[0], v[1], v[2]);
				if (heightResult.first) {
	...
	public FindNearestPolyResult findNearestPoly(float[] center, float[] extents, QueryFilter filter) {
		...
		for (int i = 0; i < polys.size(); ++i) {
			...
			float d = 0;
			float[] diff = DetourCommon.vSub(center, closestPtPoly);
			if (posOverPoly) {
	...
	protected List<Long> queryPolygonsInTile(MeshTile tile, float[] qmin, float[] qmax, QueryFilter filter) {
		...
		if (tile.data.bvTree != null) {
			...
			while (nodeIndex < end) {
				...
				boolean overlap = DetourCommon.overlapQuantBounds(bmin, bmax, node.bmin, node.bmax);
	...
	public List<Long> queryPolygons(float[] center, float[] extents, QueryFilter filter) {
		float[] bmin = DetourCommon.vSub(center, extents);
		float[] bmax = DetourCommon.vAdd(center, extents);
	...
	protected Status appendPortals(int startIdx, int endIdx, float[] endPos, List<Long> path, List<StraightPathItem> straightPath,
		...
		for (int i = startIdx; i < endIdx; i++) {
			...
			// Append intersection
			Tupple3<Boolean, Float, Float> interect = DetourCommon.intersectSegSeg2D(startPos, endPos, left, right);
			if (interect.first) {
				float t = interect.third;
				float[] pt = DetourCommon.vLerp(left, right, t);
	...
	public List<StraightPathItem> findStraightPath(float[] startPos, float[] endPos, List<Long> path, int maxStraightPath, int options) {
		...
		if (path.size() > 1) {
			float[] portalApex = DetourCommon.vCopy(closestStartPos);
			float[] portalLeft = DetourCommon.vCopy(portalApex);
			float[] portalRight = DetourCommon.vCopy(portalApex);
			int apexIndex = 0;
			int leftIndex = 0;
			int rightIndex = 0;
			...
			for (int i = 0; i < path.size(); ++i) {
				...
				if (i + 1 < path.size()) {
					...
					if (i == 0) {
						Tupple2<Float,Float> dt = DetourCommon.distancePtSegSqr2D(portalApex, left, right);
						if (dt.first < DetourCommon.sqr(0.001f))
							continue;
					}
				} else {
					// End of the path.
	...
	public MoveAlongSurfaceResult moveAlongSurface(long startRef, float[] startPos, float[] endPos, QueryFilter filter) {
		...
		// Search constraints
		float[] searchPos = DetourCommon.vLerp(startPos, endPos, 0.5f);
		...
		while (!stack.isEmpty()) {
			...
			for (int i = 0, j = curPoly.vertCount - 1; i < curPoly.vertCount; j = i++) {
				...
				if (nneis == 0) {
					...
					int vj = j * 3;
					int vi = i * 3;
					Tupple2<Float, Float> distSeg = DetourCommon.distancePtSegSqr2D(endPos, verts, vj, vi);
					float distSqr = distSeg.first;
					float tseg = distSeg.second;
					if (distSqr < bestDist) {
						// Update nearest distance.
						bestPos = DetourCommon.vLerp(verts, vj, vi, tseg);
						bestDist = distSqr;
						bestNode = curNode;
					}
				} else {
					for (int k = 0; k < nneis; ++k) {
						...
						int vj = j * 3;
						int vi = i * 3;
						Tupple2<Float, Float> distseg = DetourCommon.distancePtSegSqr2D(searchPos, verts, vj, vi);
						float distSqr = distseg.first;
	...
	protected PortalResult getPortalPoints(long from, Poly fromPoly, MeshTile fromTile, long to, Poly toPoly, MeshTile toTile,
		...
		if (link.side != 0xff) {
			...
			if (link.bmin != 0 || link.bmax != 255) {
				...
				left = DetourCommon.vLerp(fromTile.data.verts, v0 * 3, v1 * 3, tmin);
				right = DetourCommon.vLerp(fromTile.data.verts, v0 * 3, v1 * 3, tmax);
			}
		}
	...
	public RaycastHit raycast(long startRef, float[] startPos, float[] endPos, QueryFilter filter, int options, long prevRef) {
		...
		float[] dir = DetourCommon.vSub(endPos, startPos);
		...
		while (curRef != 0) {
			...
			DetourCommon.IntersectResult iresult = DetourCommon.intersectSegmentPoly2D(startPos, endPos, verts, nv);
			...
			if ((options & DT_RAYCAST_USE_COSTS) != 0) {
				...
				float[] eDir = DetourCommon.vSub(e2, e1);
				float[] diff = DetourCommon.vSub(new VectorPtr(curPos), e1);
				float s = DetourCommon.sqr(eDir[0]) > DetourCommon.sqr(eDir[2]) ? diff[0] / eDir[0] : diff[2] / eDir[2];
	...
	public FindPolysAroundResult findPolysAroundShape(long startRef, float[] verts, int nverts,
		...
		while (!m_openList.isEmpty()) {
			...
			for (int i = bestPoly.firstLink; i != NavMesh.DT_NULL_LINK; i = bestTile.links.get(i).next) {
				...
				DetourCommon.IntersectResult ir = DetourCommon.intersectSegmentPoly2D(va, vb, verts, nverts);
				if (!ir.intersects)
					continue;
		...
		for (int i = 0, j = poly.vertCount - 1; i < poly.vertCount; j = i++) {
			...
			for (int k = 1; k < ints.size(); ++k) {
				...
				if (storePortals && ints.get(k).ref != 0) {
					...
					float[] seg = new float[6];
					System.arraycopy(DetourCommon.vLerp(tile.data.verts, vj, vi, tmin), 0, seg, 0, 3);
					System.arraycopy(DetourCommon.vLerp(tile.data.verts, vj, vi, tmax), 0, seg, 3, 3);
					segmentVerts.add(seg);
	...
	public FindDistanceToWallResult findDistanceToWall(long startRef, float[] centerPos, float maxRadius,
		...
		while (!m_openList.isEmpty()) {
			...
			for (int i = 0, j = bestPoly.vertCount - 1; i < bestPoly.vertCount; j = i++) {
				...
				Tupple2<Float, Float> distseg = DetourCommon.distancePtSegSqr2D(centerPos, bestTile.data.verts, vj, vi);
				float distSqr = distseg.first;
				float tseg = distseg.second;
			...
			for (int i = bestPoly.firstLink; i != NavMesh.DT_NULL_LINK; i = bestTile.links.get(i).next) {
				...
				Tupple2<Float, Float> distseg = DetourCommon.distancePtSegSqr2D(centerPos, bestTile.data.verts, va, vb);
				float distSqr = distseg.first;
				// If the circle is not touching the next polygon, skip it.
				if (distSqr > radiusSqr)
					continue;
		...
		// Calc hit normal.
		float[] hitNormal = DetourCommon.vSub(centerPos, hitPos);
